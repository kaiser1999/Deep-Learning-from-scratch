...
class SimpleRNN(Layer):
    def __init__(self, d_ell, a_func="tanh", return_sequence=False):
        self.d_ell = d_ell
    ...
    def Forward(self, x):
        self.x = x
        self.M = np.shape(x)[-1]
        self.h = np.zeros((self.seq_len+1, self.d_ell, self.M))
        self.z_h = np.zeros((self.seq_len, self.d_ell, self.M))
        self.h[0,:] = np.repeat(self.h_0, self.M).reshape(-1, self.M)
...
#%%
class GRU(Layer):
    def __init__(self, d_ell, a_func="tanh", recurrent_func="sigmoid", 
    ...
    def Forward(self, x):
        self.x = x
        self.M = np.shape(x)[-1]  
        self.z_r = np.zeros((self.seq_len, self.d_ell, self.M))
        self.z_u = np.zeros((self.seq_len, self.d_ell, self.M))
        self.z_tilde_h = np.zeros((self.seq_len, self.d_ell, self.M))
        
        self.r = np.zeros((self.seq_len, self.d_ell, self.M))
        self.u = np.zeros((self.seq_len, self.d_ell, self.M))
        self.tilde_h = np.zeros((self.seq_len, self.d_ell, self.M))
        self.h = np.zeros((self.seq_len+1, self.d_ell, self.M))
        ...
        for t in range(self.seq_len):
            ...
            self.z_tilde_h[t,:] = (self.W_tilde_h @ x[t,:] 
                                   + self.V_tilde_h @ (self.r[t,:] * self.h[t,:]) 
                                   + self.b_tilde_h)
            self.tilde_h[t,:] = self.g_func.get_act(self.z_tilde_h[t,:])
            
            self.h[t+1,:] = ((1 - self.u[t,:]) * self.h[t,:] +
                             self.u[t,:] * self.tilde_h[t,:])
...
class LSTM(Layer):
    ...
    def Forward(self, x):
        self.x = x
        self.M = np.shape(x)[-1]
        self.z_f = np.zeros((self.seq_len, self.d_ell, self.M))
        self.z_i = np.zeros((self.seq_len, self.d_ell, self.M))
        self.z_tilde_c = np.zeros((self.seq_len, self.d_ell, self.M))
        self.z_o = np.zeros((self.seq_len, self.d_ell, self.M))
        
        self.f = np.zeros((self.seq_len, self.d_ell, self.M))
        self.i = np.zeros((self.seq_len, self.d_ell, self.M))
        self.tilde_c = np.zeros((self.seq_len, self.d_ell, self.M))
        self.o = np.zeros((self.seq_len, self.d_ell, self.M))
        self.c = np.zeros((self.seq_len+1, self.d_ell, self.M))
        self.h = np.zeros((self.seq_len+1, self.d_ell, self.M))
        self.g_c = np.zeros((self.seq_len, self.d_ell, self.M))
        
        self.c[0,:] = np.repeat(self.c_0, self.M).reshape(-1, self.M)
        self.h[0,:] = np.repeat(self.h_0, self.M).reshape(-1, self.M)
        for t in range(self.seq_len):
            ...
            self.z_tilde_c[t,:] = (self.W_tilde_c @ x[t,:] +
                                   self.V_tilde_c @ self.h[t,:] +
                                   self.b_tilde_c)
